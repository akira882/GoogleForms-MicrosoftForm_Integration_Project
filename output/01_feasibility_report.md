  ...
  "properties": {
    ...
    "metadata": {
      ...
      "properties": {
        "company": {
          "type": "string",
          "description": "M&A企業識別子（例: GMO Tech, GMO Internet）"
        },
        "department": {
          "type": "string",
          "description": "部門名"
...
| Power Automate | Microsoft 365 E3: 2,000 runs/日 | 1日あたり2,000フォーム応答まで対応可能（GMO規模では十分） |
...
| 解決策 | 技術スタック | 難易度 | メンテナンス | スケーラビリティ | GMO親和性 | 推奨度 |
|--------|------------|--------|------------|---------------|----------|--------|
...
- 月額$10〜$25と低コスト
- GMOグループの既存インフラ（Microsoft 365）を活用
...
- GMOグループのAI活用方針と完全整合
- GMO AIブースト支援金（最大500万円）の活用可能

**デメリット:**
- Claude API コスト: 月間$50〜$100（ただしGMO AIブースト支援金でカバー可能）
...
| セキュリティ侵害 | 低 (5%) | 高 | IAM、暗号化、監査ログ | GMOトラスト・ログインSSO |
...
- ✅ Claude API 予算確保（GMO AIブースト支援金申請承認）
- ✅ 個人情報匿名化設計の法務部門承認
...
| GMOトラスト・ログイン | SAML SSO連携 | 統一認証、セキュリティ強化 |
| GMO jobbi | BigQuery連携 | 求人広告効果測定とのクロス分析 |
...
**この提案の貢献:**
- Phase 3でClaude APIを活用 → GMOグループAI戦略と整合
- 年間5,000〜10,000時間の追加削減見込み
- GMO AIブースト支援金（最大500万円）の活用
...
6. ✅ **GMOグループの既存インフラを活用:** 追加コストを最小化

**残る5%の不確実性:**

- Power Automateの予期しない仕様変更（ただし公式製品のため可能性は極めて低い）
- GMOグループ内の政治的要因（既存システムとの競合等）
...
4. **Month 4-6:** Phase 2実装、GMO jobbi連携
